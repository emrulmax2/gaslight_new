...
class HomeOwnerGasSafetyController extends Controller
{
          ...
          try {
            ...
            if($customer_job_id > 0) {
                ...
                return response()->json([
                    ...
                    'data' =>  GasSafetyRecord::with(['customer','signature','appliance'])->findOrFail($gasSafetyRecord->id),
                ], 200);
            } else {
                return response()->json([
                    'success' => false,
    ...
    public function getDetails($gas_safety_id)
    {
        ...
        try {
            ...
            if(empty($gsr->certificate_number)):
                ...
                $userLastCertificate = GasSafetyRecord::where('job_form_id', $form->id)->where('created_by', $user_id)->where('id', '!=', $gsr->id)->orderBy('id', 'DESC')->get()->first();
            ...
            return response()->json([
                ...
                'data' => [
                    'form' => $form,
                    'gas_safety_record' => $gsr,
                    'gas_safety_record1' => GasSafetyRecordAppliance::where('gas_safety_record_id', $gsr->id)->where('appliance_serial', 1)->get()->first(),
                    'gas_safety_record2' => GasSafetyRecordAppliance::where('gas_safety_record_id', $gsr->id)->where('appliance_serial', 2)->get()->first(),
                    'gas_safety_record3' => GasSafetyRecordAppliance::where('gas_safety_record_id', $gsr->id)->where('appliance_serial', 3)->get()->first(),
                    'gas_safety_record4' => GasSafetyRecordAppliance::where('gas_safety_record_id', $gsr->id)->where('appliance_serial', 4)->get()->first(),
    ...
    public function approve($gsr_id)
    {
        try {
            ...
            return response()->json([
                    'success' => true,
                    'message' => 'Gas Safety Record successfully approved.',
                    'gsr_id' => $gasSafetyRecord->id
                ], 200);
        } catch (ModelNotFoundException $e) {
            return response()->json([
                'success' => false,
                'message' => 'Gas safety record not found. . The requested Gas safety record (ID: '.$gsr_id.') does not exist or may have been deleted.',
            ], 404);
    ...
    public function approve_email($gsr_id)
    {
        try {
            ...
            return response()->json([
                'success' => true,
                'message' => $emailSent 
                    ? 'Gas safety record Certificate has been approved and emailed to the customer'
                    : 'Gas safety record Certificate has been approved but email failed: ' . 
