    ...
    public function getDetails($invoice_id){
        try {
            ...
            if(empty($inv->invoice_number)):
                ...
                $userLastInvoice = Invoice::where('customer_job_id', $invoice->customer_job_id)->where('job_form_id', $form->id)->where('id', '!=', $invoice->id)->where('created_by', $user_id)->orderBy('id', 'DESC')->get()->first();
