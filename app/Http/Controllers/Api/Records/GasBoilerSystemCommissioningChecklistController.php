...
class GasBoilerSystemCommissioningChecklistController extends Controller
{
    ...
    public function store(Request $request){
        ...
        if($customer_job_id > 0):
            ...
            return response()->json([
                'success' => true,
                'message' => $gasBoilerSCC->wasRecentlyCreated ? 'Certificate successfully created' : 'Certificate successfully updated', 
                'data' => GasBoilerSystemCommissioningChecklist::with(['customer', 'customer.contact', 'job', 'job.property', 'form', 'user', 'user.company', 'signature'])->findOrFail($gasBoilerSCC->id)
            ], 200);
        else:
    ...
    public function getDetails(Request $request, $checklist_id){
        try {
            ...
            if(empty($boilerCommChecklist->certificate_number)):
                ...
                $userLastCertificate = GasBoilerSystemCommissioningChecklist::where('job_form_id', $form->id)->where('created_by', $user_id)->where('id', '!=', $boilerCommChecklist->id)->orderBy('id', 'DESC')->get()->first();
                ...
                GasBoilerSystemCommissioningChecklist::where('id', $boilerCommChecklist->id)->update(['certificate_number' => $certificateNumber]);
            endif;

            $thePdf = $this->generatePdf($boilerCommChecklist->id);

            return response()->json([
                    'success' => true,
                    'data' => [
                        'form' => $form,
                        'boilerCommChecklist' => $boilerCommChecklist,
                        'boiler_comm_checklist_appliance1' => GasBoilerSystemCommissioningChecklistAppliance::where('gas_boiler_system_commissioning_checklist_id', $boilerCommChecklist->id)->where('appliance_serial', 1)->get()->first(),
                        'signature' => $boilerCommChecklist->signature ? Storage::disk('public')->url($boilerCommChecklist->signature->filename) : '',
                        'pdf_url' => $thePdf
                    ]
                ], 200);
            ...
            return response()->json([
                'success' => false,
                'message' => 'Gas boiler system commissioning checklist not found. The requested gas boiler system commissioning checklist (ID: '.$checklist_id.') does not exist or may have been deleted.',
            ], 404);
    ...
    public function approve($checklist_id)
    {
        try {
            ...
            return response()->json([
                    'success' => true,
                    'message' => 'Gas boiler system commissioning checklist successfully approved.',
                    'gbscc_id' => $gbscc->id
                ], 200);
        } catch (ModelNotFoundException $e) {
            return response()->json([
    ...
    public function approve_email(Request $request, $checklist_id)
    {
        try {
            ...
            $updateResult = $gbscc->update([
                'status' => 'Approved & Sent'
            ]);

            if (!$updateResult) {
                throw new \Exception("Failed to update gas service record status");
            }
            ...
            return response()->json([
                'success' => true,
                'message' => $emailSent 
                    ? 'Gas boiler system commissioning checklist has been approved and emailed to the customer'
                    : 'Gas boiler system commissioning checklist has been approved but email failed: ' . 
    ...
    public function download($checklist_id)
    {
        try {
            $gbscc = GasBoilerSystemCommissioningChecklist::findOrFail($checklist_id);
