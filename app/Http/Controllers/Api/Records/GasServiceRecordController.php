...
class GasServiceRecordController extends Controller
{
    ...
    public function store(Request $request)
    {
        try {
            ...
            return response()->json([
                'success' => true,
                'message' => $gasServiceRecord->wasRecentlyCreated ? 'Certificate successfully created' : 'Certificate successfully updated',
                'data' => GasServiceRecord::with(['customer', 'customer.contact', 'job', 'job.property', 'form', 'user', 'user.company', 'signature'])->findOrFail($gasServiceRecord->id)
            ], 200);
    ...
    public function getDetails(Request $request, $record_id){
        try {
            ...
            if(empty($gasServiceRecord->certificate_number)):
                ...
                $userLastCertificate = GasServiceRecord::where('job_form_id', $form->id)->where('created_by', $user_id)->where('id', '!=', $gasServiceRecord->id)->orderBy('id', 'DESC')->get()->first();
                ...
                GasServiceRecord::where('id', $gasServiceRecord->id)->update(['certificate_number' => $certificateNumber]);
            endif;

            $thePdf = $this->generatePdf($gasServiceRecord->id);

          return response()->json([
                'success' => true,
                'data' => [
                    'form' => $form,
                    'gas_service_record' => $gasServiceRecord,
                    'gas_service_record1' => GasServiceRecordAppliance::where('gas_service_record_id', $gasServiceRecord->id)->where('appliance_serial', 1)->get()->first(),
                    'signature' => $gasServiceRecord->signature ? Storage::disk('public')->url($gasServiceRecord->signature->filename) : '',
                    'pdf_url' => $thePdf
                ]
            ], 200);
     ...
     public function approve($record_id)
    {
        try {
            ...
            $gasServiceRecord->update([
                'status' => 'Approved'
            ]);

            return response()->json([
                    'success' => true,
                    'message' => 'Gas service record successfully approved.',
                    'gwn_id' => $gasServiceRecord->id
                ], 200);
        } catch (ModelNotFoundException $e) {
            return response()->json([
                'success' => false,
    ...
    public function approve_email(Request $request, $record_id)
    {
        try {
            ...
            $updateResult = $gasServiceRecord->update([
            ...
            return response()->json([
                'success' => true,
                'message' => $emailSent 
                    ? 'Gas service record Certificate has been approved and emailed to the customer'
                    : 'Gas service record Certificate has been approved but email failed: ' . 
