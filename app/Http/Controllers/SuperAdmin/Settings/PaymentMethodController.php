...
class PaymentMethodController extends Controller
{
    public function index(){
        return view('app.superadmin.settings.payment-method.index',[
            'title' => 'Site Settings - Gas Certificate APP',
            'subtitle' => 'Payment Methods',
            'breadcrumbs' => [
                ['label' => 'Site Settings', 'href' => route('superadmin.site.setting')],
                ['label' => 'Payment Method', 'href' => 'javascript:void(0);'],
            ]
        ]);
    }
     ...
     public function list(Request $request) {
        ...
        $query = PaymentMethod::orderByRaw(implode(',', $sorts));
        if(!empty($queryStr)):
    ...
    {
        $referral_code = PaymentMethod::create([
            'name' => $request->name,
    ...
    {
        $referral_code = PaymentMethod::find($request->id);
    ...
    public function destroy($method_id){
        $package = PaymentMethod::find($method_id)->delete();

        return response()->json(['msg' => 'Payment method successfully deleted.', 'red' => ''], 200);
    }

    public function restore($method_id){
        $package = PaymentMethod::where('id', $method_id)->withTrashed()->restore();
