...
class GasUnventedHotWaterCylinderRecordController extends Controller
{
    ...
    public function show(GasUnventedHotWaterCylinderRecord $guhwcr){
        ...
        if(empty($guhwcr->certificate_number)):
            ...
            $userLastCertificate = GasUnventedHotWaterCylinderRecord::where('job_form_id', $form->id)->where('created_by', $user_id)->where('id', '!=', $guhwcr->id)->orderBy('id', 'DESC')->get()->first();
    ...
    public function storeNew(Request $request){
        ...
        $unventedSystems = json_decode($request->unventedSystems);
        $inspectionRecords = json_decode($request->inspectionRecords);
    ...
    public function editReady(Request $request){
        ...
        $record = GasUnventedHotWaterCylinderRecord::with('customer', 'customer.contact', 'job', 'job.property')->find($record_id);
        $unventedSystems = GasUnventedHotWaterCylinderRecordSystem::where('gas_unvented_hot_water_cylinder_record_id', $record_id)->orderBy('id', 'desc')->get()->first();
        $inspectionRecords = GasUnventedHotWaterCylinderRecordInspection::where('gas_unvented_hot_water_cylinder_record_id', $record_id)->orderBy('id', 'desc')->get()->first();
        ...
        $data['systemAnswered'] = count(array_filter($data['unventedSystems'], function($v) { return !empty($v); }));
        $data['inspectionAnswered'] = count(array_filter($data['inspectionRecords'], function($v) { return !empty($v); }));
