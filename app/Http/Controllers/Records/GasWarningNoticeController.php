...
class GasWarningNoticeController extends Controller
{
    ...
    public function show(GasWarningNotice $gsr){
        ...
        if(empty($gsr->certificate_number)):
            ...
            $userLastCertificate = GasWarningNotice::where('job_form_id', $form->id)->where('created_by', $user_id)->where('id', '!=', $gsr->id)->orderBy('id', 'DESC')->get()->first();
    ...
    public function store(Request $request){
        ...
        if($submit_type == 2):
            ...
            GasWarningNotice::where('id', $gwn_id)->update($data);
            $message = 'Gas Warning Noteice Certificate successfully approved.';
        endif;
    ...
    public function storeNew(Request $request){
        ...
        if($customer_job_id > 0):
            ...
            return response()->json(['msg' => 'Certificate successfully created.', 'red' => route('new.records.gas.warning.notice.show', $gasWarningRecord->id)], 200);
        else:
