...
class LandlordGasSafetyController extends Controller
{
    ...
    public function show(GasLandlordSafetyRecord $glsr){
        ...
        if(empty($glsr->certificate_number)):
            ...
            $userLastCertificate = GasLandlordSafetyRecord::where('job_form_id', $form->id)->where('created_by', $user_id)->where('id', '!=', $glsr->id)->orderBy('id', 'DESC')->get()->first();
                        ...
                        $PDFHTML .= '<tbody>';
                            $PDFHTML .= '<tr>';
                                ...
                                $PDFHTML .= '<td class="w-col8 text-center align-middle px-5">';
                                    $PDFHTML .= '<h1 class="text-white text-xl leading-none mt-0 mb-05">CP12 Landlord Gas Safety Record</h1>';
