...
class InspectionNotificationController extends Controller
{
    ...
    // Expiry date
    // Email sent status. & send on date
    // Send or Resend. 

    public function show($date = null){
        $monthYear = $date ?? date('m-Y');
        $theDate = Carbon::createFromFormat('m-Y', $monthYear);

        return view('app.records.inspections.show', [
            'title' => 'Next Inspections - Gas Certificate APP',
            'breadcrumbs' => [
                ['label' => 'Upcoming Inspections', 'href' => 'javascript:void(0);'],
                ['label' => 'Reminder for '.$theDate->monthName.', '.$theDate->year, 'href' => 'javascript:void(0);'],
            ],
            'result' => $this->monthlyList($monthYear)
        ]);
    }

    public function monthlyList($date){
        ...
        foreach ($records as $record) {
            ...
            $events[] = [
                ...
                'job_address' => $record->property->full_address ?? '',
