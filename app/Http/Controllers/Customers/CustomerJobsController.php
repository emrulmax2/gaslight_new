...
class CustomerJobsController extends Controller
{
    ...
    public function list(Request $request) {
        ...
        $query = CustomerJob::with('customer', 'property', 'priority', 'status', 'calendar', 'calendar.slot')->orderByRaw(implode(',', $sorts))
        ...
        if(!empty($Query)):
            foreach($Query as $list):
                ...
                $html .= '<a data-id="'.$list->id.'" href="'.route('customer.jobs.edit', [$list->customer_id, $list->id]).'" class="'.$itemBG.' box relative jobItemWrap px-3 py-3 rounded-md block sm:flex w-full items-center mb-1">';
