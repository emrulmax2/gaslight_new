...
@section('subcontent')
    ...
    <form method="post" action="#" id="gasJobSheetRecordForm">
        <div class="grid grid-cols-11 gap-x-6 pb-20 mt-5">
            <div class="intro-y col-span-12 max-sm:mt-5 sm:col-span-2 order-2 sm:order-1">
                <div class="sticky top-0">
                    <div class="flex flex-col justify-center items-center shadow-md rounded-md bg-white p-5">
                        <x-base.button as="a" href="{{ route('records', [$form->slug, $gjsr->customer_job_id]) }}" class="w-full mb-2 border-0 cursor-pointer text-slate-500 shadow-none [&.active]:bg-[#164e63] [&.active]:text-white hover:bg-[#164e63] focus:bg-[#164e63] hover:text-white focus:text-white">
            ...
            <input id="customer_job_id" name="customer_job_id" type="hidden" value="{{ $gjsr->customer_job_id }}" />
            <input id="customer_id" name="customer_id" type="hidden" value="{{ $gjsr->customer_id }}" />
            <input id="job_form_id" name="job_form_id" type="hidden" value="{{ $form->id }}" />
            <input id="gjsr_id" name="gjsr_id" type="hidden" value="{{ $gjsr->id }}" />
...
@pushOnce('scripts')
    @vite('resources/js/app/records/job_sheet_record_show.js')
@endPushOnce