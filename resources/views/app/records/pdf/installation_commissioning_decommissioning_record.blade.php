<html>
    ...
    <body>
                ...
                <tbody>
                    <tr>
                        ...
                        <td class="w-col8 text-center align-middle px-5">
                            <h1 class="text-white text-xl leading-none mt-0 mb-05">Installation / Commissioning / Decommissioning Record</h1>
                            <div class="text-white text-12px leading-1-3">
                                This Safety Record can be used to document the outcomes of the checks and tests required by The Gas Safety (Installation and Use) regulations.
        ...
        <table class="table table-sm bordered border-primary mt-1-5">
            <thead>
                <tr>
                    <th colspan="{{ ($worktypes->count() > 0 ? $worktypes->count() : 1) }}" class="whitespace-nowrap border-primary border-b-white border-b-1 bg-primary text-white text-11px uppercase leading-none px-2 py-1 align-middle text-left">
                        Work Type
                    </th>
                </tr>
            ...
            <tbody>
                @php 
                    $appWorkTypes = !empty($record->available_options->appliances->work_type) ? $record->available_options->appliances->work_type : [];
                @endphp
                @if($worktypes->count() > 0)
                    <tr>
                        @foreach($worktypes as $wtp)
                            <td class="border-primary text-primary pl-2 pr-2 py-05 text-13px tracking-normal text-center leading-1-2 border-b border-r">
                                {!! (!empty($appWorkTypes) && in_array($wtp->id, $appWorkTypes) ? '<div style="font-family: DejaVu Sans, sans-serif; font-weight: normal;">✔</div>' : '<div style="font-family: DejaVu Sans, sans-serif; font-weight: normal;">✗</div>') !!}
                            </td>
                        @endforeach
                    </tr>
                @endif
        ...
        <table class="table table-sm bordered border-primary mt-1-5">
            <thead>
                <tr>
                    <th class="whitespace-nowrap border-primary border-b-white border-b-1 border-r border-r-sec bg-primary text-white text-11px uppercase leading-none px-2 py-1 align-middle text-left">
                        Is the gas installation/appliance(s) safe to use
                    </th>
                    <th class="whitespace-nowrap border-primary border-b-white border-b-1 bg-primary text-white text-11px uppercase leading-none px-2 py-1 align-middle text-left">
                        Have warning labels been affixed?
                    </th>
                </tr>
            </thead>
            <tbody>
