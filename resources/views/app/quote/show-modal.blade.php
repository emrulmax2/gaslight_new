        ...
        <form method="post" action="#" id="sendEmailForm" enctype="multipart/form-data">
            ...
            <x-base.dialog.description class="p-5">
                <div class="mb-3">
                    ...
                    @if(!isset($quote->customer->contact->email) || empty($quote->customer->contact->email))
                        <x-base.form-input value="{{ (isset($quote->customer->contact->email) && !empty($quote->customer->contact->email) ? $quote->customer->contact->email : '') }}" name="customer_email" class="w-full" type="text" placeholder="example@exm.com" />
                    @else
                        <x-base.form-input readonly value="{{ (isset($quote->customer->contact->email) && !empty($quote->customer->contact->email) ? $quote->customer->contact->email : '') }}" name="customer_email" class="w-full" type="text" placeholder="example@exm.com" />
                    @endif
                ...
                <div class="mb-3">
                    ...
                    <x-base.form-input value="{{ (isset($quote->email_template->cc_email_address) ? $quote->email_template->cc_email_address : '') }}" name="cc_email_address" class="w-full" type="text" placeholder="example@exm.com,example@exm.com...." />
                ...
                <div class="mb-3">
                    ...
                    <x-base.form-input id="subject" value="{{ (isset($quote->email_template->subject) ? $quote->email_template->subject : '') }}" name="subject" class="w-full" type="text" placeholder="{{ $form->name }} for job at [jobaddr1] [jobaddr2]" />
                ...
                <div>
                    <div class="editor" id="theEditor">{!! (isset($quote->email_template->content) ? $quote->email_template->content : '') !!}</div>
