                    ...
                    <div>
                        @if(isset($form->childs) && $form->childs->count() > 0)
                            @foreach($form->childs as $child)
                                <a href="{{ route('new.records.create', $child->id) }}" class="flex w-full border-b border-b-slate-200/60 bg-white px-3 py-2 hover:text-primary justify-start items-center">
...
@pushOnce('scripts')
    @vite('resources/js/app/new-records/index.js')
@endPushOnce