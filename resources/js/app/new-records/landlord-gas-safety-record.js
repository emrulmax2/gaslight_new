...
(function(){
    ...
    $('.commentsBlock').on('click', function(e){
        ...
        document.getElementById("commentsModal").addEventListener("shown.tw.modal", function (event) {
            if(localStorage.glsrComments){
                let glsrComments = localStorage.getItem('glsrComments');
                let glsrCommentsObj = JSON.parse(glsrComments);

                if(!$.isEmptyObject(glsrCommentsObj)){
                    for (const [key, value] of Object.entries(glsrCommentsObj)) {
    ...
    $('#commentsForm').on('submit', function(e){
        ...
        localStorage.removeItem('glsrComments');
        localStorage.setItem('glsrComments', JSON.stringify(formated_data));
    ...
    $('#certificateForm').on('submit', function(e){
        ...
        if(localStorage.glsrComments){
            let glsrComments = localStorage.getItem('glsrComments');
            formData.append('glsrComments', glsrComments);
        }

        if($.isEmptyObject(errors)){
            axios({
                method: "post",
                url: route('new.records.glsr.store.new'),
                data: formData,
