...
(function(){
    ...
    if(localStorage.invoiceNumber){
        ...
        axios({
            method: "post",
            url: route('new.records.get.invoice.number'),
            data: {job_form_id : job_form_id},
            headers: {'X-CSRF-TOKEN' :  $('meta[name="csrf-token"]').attr('content')},
        }).then(response => {
            if (response.status == 200) {
                let invoiceNumber = response.data.invoiceNumber;
    ...
    $('#certificateForm').on('submit', function(e){
        ...
        let errors = {};
        if($theForm.find('[name="invoice_number"]').val() == 0 || $theForm.find('[name="invoice_number"]').val() == ''){
            errors['invoice_number'] = 'Invoice Number not generated.&nbsp;';
        }
        ...
        if(localStorage.invoiceItems){
            let invoiceItems = localStorage.getItem('invoiceItems');
            formData.append('invoiceItems', invoiceItems);
        }else{
            errors['item_error'] = 'Please add at least one item for this invoice.&nbsp;';
        }
        if(localStorage.invoiceDiscounts){
            let invoiceDiscounts = localStorage.getItem('invoiceDiscounts');
            formData.append('invoiceDiscounts', invoiceDiscounts);
        }
        if(localStorage.invoiceAdvance){
            let invoiceAdvance = localStorage.getItem('invoiceAdvance');
            formData.append('invoiceAdvance', invoiceAdvance);
        }
        if(localStorage.invoiceNotes){
            let invoiceNotes = localStorage.getItem('invoiceNotes');
            formData.append('invoiceNotes', JSON.parse(invoiceNotes));
        }

        if($.isEmptyObject(errors)){
            axios({
                method: "post",
                url: route('invoice.store.new'),
                data: formData,
                headers: {'X-CSRF-TOKEN' :  $('meta[name="csrf-token"]').attr('content')},
            }).then(response => {
                if (response.status == 200) {
