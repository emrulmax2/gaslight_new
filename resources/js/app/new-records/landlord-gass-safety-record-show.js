...
(function(){
    ...
    $('#gasSafetyRecordForm').on('submit', function(e){
        ...
        let glsr_id = $theForm.find('#glsr_id').val();
        ...
        axios({
            method: "post",
            url: route('new.records.glsr.store', [form_slug, glsr_id]),
            data: form_data,
    ...
    $('.editRecordBtn').on('click', function(e){
        ...
        axios({
            method: "post",
            url: route('new.records.glsr.edit.ready.new'),
            ...
            if (response.status == 200) {
                ...
                localStorage.setItem('glsrComments', JSON.stringify(row.glsrComments));
