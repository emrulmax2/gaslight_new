...
(function(){
    ...
    $('.form-wizard-next-btn').on('click', function () {
        ...
        if(parentFieldset.index() == 0){
            ...
            url = route('records.guhwcr.store.system');
        }else if(parentFieldset.index() == 3){
            url = route('records.guhwcr.store.inspection');
        }else if(parentFieldset.index() == 4){
            url = route('records.guhwcr.store.signatures');
        }
    ...
    $('#signatureForm').on('submit', function(e){
        ...
        let form_data = new FormData(form);
        let url = route('records.guhwcr.store.signatures');
